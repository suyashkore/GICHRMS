  <div class="modal fade" data-backdrop="static" id="add_members_modal" tabindex="-1" role="dialog">
    <form method="POST" id="members_form_add">
      <div class="modal-dialog" role="any">
        ...
        <div class="form-group">
              <label class="mtop20">Client Member</label>
              <select data-none-selected-text="<?= _l('chat_non_selected_member_text'); ?>" data-live-search="true" id="client_members" name="client_members[]" multiple class="form-control">
                <?php
                if (is_array($client) && !empty($client)) {
                  foreach ($client as $member) {
                   ?>
                  <option data-subtext="<?= ($member['admin'] ? 'admin' : ''); ?>" data-icon="fa fa-user-o" value="<?= $member['userid'] ?>"><?= $member['firstname'] . ' ' . $member['lastname']; ?>
                </option>
              <?php }
            } ?>
          </select>
