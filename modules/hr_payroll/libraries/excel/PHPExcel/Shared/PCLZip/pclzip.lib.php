...
class PclZip
{
    ...
    public function create($p_filelist)
    {
        ...
        if (is_array($p_filelist)) {
            ...
            $v_string_list = new_explode(PCLZIP_SEPARATOR, $p_filelist);
        } else {
        ...
        if ($v_size > 0) {
            ...
            if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
                ...
                if (isset($v_options[PCLZIP_OPT_ADD_PATH])) {
                    ...
                    if ((new_strlen($v_path) > 0) && (substr($v_path, -1) != '/')) {
                        $v_path .= '/';
                    }
            ...
            switch ($p_options_list[$i]) {
                ...
                case PCLZIP_OPT_BY_INDEX:
                    ...
                    if (is_string($p_options_list[$i+1])) {
                        ...
                        // ----- Parse items
                        $v_work_list = new_explode(",", $p_options_list[$i+1]);
                    ...
                    for ($j=0; $j<sizeof($v_work_list); $j++) {
                        // ----- Explode the item
                        $v_item_list = new_explode("-", $v_work_list[$j]);
    ...
    public function privAddFile($p_filedescr, &$p_header, &$p_options)
    {
        ...
        */
        ...
        $p_header['filename_len'] = new_strlen($p_filename);
    ...
    // TBC : Removed        $p_header['stored_filename'] = $v_stored_filename;
        ...
        if ($p_filedescr['type']=='file') {
            ...
            $p_header['external'] = 0x00000000;
            $p_header['size'] = new_strlen($p_filedescr['content']);
        }
        ...
        if (isset($p_filedescr['comment'])) {
            $p_header['comment_len'] = new_strlen($p_filedescr['comment']);
    ...
    //            eval('$v_result = '.$p_options[PCLZIP_CB_PRE_ADD].'(PCLZIP_CB_PRE_ADD, $v_local_header);');
        ...
        if (new_strlen($p_header['stored_filename']) > 0xFF) {
            ...
            if ($p_filedescr['type'] == 'file') {
                ...
                if ((!isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF])) && (isset($p_options[PCLZIP_OPT_TEMP_FILE_ON]) || (isset($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD]) && ($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD] <= $p_header['size'])))) {
                    ...
                    if ($p_options[PCLZIP_OPT_NO_COMPRESSION]) {
                        ...
                        // ----- Set header parameters
                        $p_header['compressed_size'] = new_strlen($v_content);
            ...
            if ($p_remove_all_dir) {
                ...
                if ($v_compare > 0) {
                    if ($v_compare == 2) {
                        ...
                        $v_stored_filename = substr($v_stored_filename, new_strlen($p_remove_dir));
                    }
                }
            }
    ...
    public function privWriteFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvvVVVvv", 0x04034b50, $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], new_strlen($p_header['stored_filename']), $p_header['extra_len']);

        // ----- Write the first 148 bytes of the header in the archive
        fputs($this->zip_fd, $v_binary_data, 30);

        // ----- Write the variable fields
        if (new_strlen($p_header['stored_filename']) != 0) {
            fputs($this->zip_fd, $p_header['stored_filename'], new_strlen($p_header['stored_filename']));
        }
    ...
    public function privWriteCentralFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvvvVVVvvvvvVV", 0x02014b50, $p_header['version'], $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], new_strlen($p_header['stored_filename']), $p_header['extra_len'], $p_header['comment_len'], $p_header['disk'], $p_header['internal'], $p_header['external'], $p_header['offset']);
    ...
    public function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries, $p_nb_entries, $p_size, $p_offset, new_strlen($p_comment));

        // ----- Write the 22 bytes of the header in the zip file
        fputs($this->zip_fd, $v_binary_data, 22);

        // ----- Write the variable fields
        if (new_strlen($p_comment) != 0) {
            fputs($this->zip_fd, $p_comment, new_strlen($p_comment));
        }
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        if (($p_path != "./") && ($p_path != "/")) {
            ...
            while (substr($p_path, -1) == "/") {
                $p_path = substr($p_path, 0, new_strlen($p_path)-1);
            }
        }
        ...
        $p_remove_path_size = new_strlen($p_remove_path);
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        ...
                        if ((new_strlen($v_header['stored_filename']) > new_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (substr($v_header['stored_filename'], 0, new_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_extract = true;
                        }
    ...
    public function privReadFileHeader(&$p_header)
    {
        ...
        if (new_strlen($v_binary_data) != 26) {
            $p_header['filename'] = "";
            $p_header['status'] = "invalid_header";

            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid block size : ".new_strlen($v_binary_data));
    ...
    public function privReadCentralFileHeader(&$p_header)
    {
        ...
        // ----- Look for invalid block size
        if (new_strlen($v_binary_data) != 42) {
            $p_header['filename'] = "";
            $p_header['status'] = "invalid_header";
    ...
    public function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        // ----- Look for invalid block size
        if (new_strlen($v_binary_data) != 18) {
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid End of Central Dir Record size : ".new_strlen($v_binary_data));
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        // ----- Look if the directory is in the filename path
                        if ((new_strlen($v_header_list[$v_nb_extracted]['stored_filename']) > new_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, new_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
    ...
    public function privDirCheck($p_dir, $p_is_dir = false)
    {
        ...
        if (($p_is_dir) && (substr($p_dir, -1)=='/')) {
            $p_dir = substr($p_dir, 0, new_strlen($p_dir)-1);
        }
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if (($p_dir == '.') || ((new_strlen($p_dir) >=2) && (substr($p_dir, 0, 2) == './'))) {
        $p_dir = PclZipUtilTranslateWinPath(getcwd(), false).'/'.substr($p_dir, 1);
    }
    if (($p_path == '.') || ((new_strlen($p_path) >=2) && (substr($p_path, 0, 2) == './'))) {
        $p_path = PclZipUtilTranslateWinPath(getcwd(), false).'/'.substr($p_path, 1);
    }

    // ----- Explode dir and path by directory separator
    $v_list_dir = new_explode("/", $p_dir);
    $v_list_dir_size = sizeof($v_list_dir);
    $v_list_path = new_explode("/", $p_path);
