...
class Hr_payslip_merge_fields extends App_merge_fields
{
	...
	public function format($payslip_detail_id)
	{
		...
		$fields['{overtime_hours}'] = $payslip_detail->overtime_hours;
		...
		$fields['{project_salary}'] = currency_converter_value($payslip_detail->project_salary, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
		$fields['{project_bonus}'] = currency_converter_value($payslip_detail->project_bonus, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
		...
		if(is_array($json_data)){
			foreach ($json_data as $key => $value) {
				if(preg_match('/^st1_/', $key) ){
					...
					$value = currency_converter_value($value, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
				}elseif(preg_match('/^13th_month_salary/', $key) ){
					
					$value = currency_converter_value($value, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
				}elseif(preg_match('/^hourly_rate/', $key) ){
					
					$value = currency_converter_value($value, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
				}elseif(preg_match('/^basic_salary/', $key) ){
					
					$value = currency_converter_value($value, $hrp_payslip->to_currency_rate, $hrp_payslip->to_currency_name ?? '', true);
				}
				if($key == 'account_number' || $key == 'epf_no'){
					$fields['{'.$key.'}'] = $value;
				}else{
					
					if (in_array($key, $payroll_system_columns_dont_format)) {
						$fields['{'.$key.'}'] = $value;
					} else {
						if(is_numeric($value)){
							$fields['{'.$key.'}'] = number_format((float)$value, 2);
						}else{
							$fields['{'.$key.'}'] = $value;
						}
					}
