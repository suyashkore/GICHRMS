...
class Hr_payroll_model extends App_Model {
    ...
    public function update_earnings_list($data)
	{
		...
		if(isset($earnings_list_hs)){
			...
			foreach ($incometax_rate_detail as $key => $value) {
				//only get row has value not zero
				if($value[0] != '' ){
					$es_detail[] = array_combine($header, $value);
	...
	public function update_data_integration($data)
	{
		...
		$this->db->or_where('option_name', 'notify_closing_payslip');
...
";
	...
	{	
		...
		$staff_information[] = 'designation';
		$staff_information[] = 'sex';
		$staff_information[] = 'birthday';
		$staff_information[] = 'datecreated';
		$staff_information[] = 'nation';
		$staff_information[] = 'passport_no';
		$staff_information[] = 'passport_expiry';
		$staff_information[] = 'emirates_ID_No';
		$staff_information[] = 'EID_expiry';
		$staff_information[] = 'labour_card_No';
		$staff_information[] = 'phonenumber';
		$staff_information[] = 'contact_person_home_country';
		$staff_information[] = 'home_contact_No';
		$staff_information[] = 'other_contact_person';
		$staff_information[] = 'other_contact_No';
		$staff_information[] = 'visa_validity';
		...
		foreach ($staff_information as $value) {
			if($value == 'staff_id'){
				...
				$staff_information_header[] = _l('mobile_no');
			}elseif($value == 'datecreated'){
				$staff_information_header[] = _l('date_of_joining');
			}elseif($value == 'sex'){
				$staff_information_header[] = _l('hrp_sex');
			}else{
				$staff_information_header[] = _l($value);
			}

			if($value == 'birthday' || $value == 'datecreated' || $value == 'passport_expiry' || $value == 'EID_expiry' ){
			...
			foreach ($hrp_employees_detail as $key => $value) {				
				...
				foreach ($combine_data as $combine_key => $combine_value) {
					if($rel_type == 'hr_records'){
						...
						}elseif($combine_key == 'datecreated'){
							$temp[$combine_key] = date('Y-m-d', strtotime($combine_value));
...
/*TO DO: XỬ LÝ CẬP NHẬT, cập nhật thông tin nhân viên sau khi người dùng thực hiện thay đổi trên handsome table, dùng cho 2 trường hợp: có tích hợp HR records module, không tích hợp Hr records module.  đang xử lý!!!*/
	...
	public function synchronization_attendance($data)
	{
		...
		if(isset($hrp_attendance_value)){
			...
			foreach ($hrp_attendance_detail as $key => $value) {
				...
				for ($i=1; $i <=31 ; $i++) { 
					$combine_temp['day_'.$i] = '0.00';
				}
				$combine_temp['actual_workday'] = '0.00';
				$combine_temp['actual_workday_probation'] = '0.00';
				$combine_temp['paid_leave'] = '0.00';
				$combine_temp['unpaid_leave'] = '0.00';
				$combine_temp['overtime_hours'] = '0.00';
		...
		if(count($employees_timesheet_leaves) > 0){
			...
			foreach ($employees_timesheet_leaves as $key => $value) {
				if(isset($staff_timesheet_leave_data[$value['staff_id']])){
					$update_batch_employees_timesheet[$key]['id'] = $value['id'] ;
					$update_batch_employees_timesheet[$key] = array_merge($update_batch_employees_timesheet[$key], $staff_timesheet_leave_data[$value['staff_id']]);
					$this->db->where('id', $value['id']);
					$this->db->update(db_prefix().'hrp_employees_timeshee_leaves', $update_batch_employees_timesheet[$key]);

				}
			}
			if(1==2){
				if(count($update_batch_employees_timesheet) > 0){
					$this->db->update_batch(db_prefix().'hrp_employees_timeshee_leaves', $update_batch_employees_timesheet, 'id');
				}
			}
		}else{
			foreach ($staff_timesheet_leave_data as $key => $batch_employees_timesheet) {
				...
				$insert_batch_employees_timesheet[] = $staff_timesheet_leave_data[$key];

				$this->db->insert(db_prefix().'hrp_employees_timeshee_leaves', $staff_timesheet_leave_data[$key]);
			}
			if(1==2){
				if(count($insert_batch_employees_timesheet) > 0){
					$this->db->insert_batch(db_prefix().'hrp_employees_timeshee_leaves', $insert_batch_employees_timesheet);
				}
			}
		}
	...
	public function hrp_get_timesheets_data($month, $rel_type)
	{
		...
		$overtime_sql_where = "SELECT creator as staff_id, date_format(additional_day, '%d') as date_work, timekeeping_value as value FROM ".db_prefix()."timesheets_additional_timesheet
		where date_format(additional_day,'%Y-%m') = '".$y_month."' AND timekeeping_type = 'OT' AND status = 1
		;";
		...
		$overtimes = $this->db->query($overtime_sql_where)->result_array();
			...
			if(in_array($timesheet['type'], $actual_workday)){
				...
				if(isset($employees_data[$timesheet['staff_id']]) && !is_null($employees_data[$timesheet['staff_id']]['probationary_expiration'])){
					...
					if($payslip_month == $probationary_expiration_month ){
						...
						//check probationary expiration day <= timesheet day
						...
						// probationary expiration day > timesheet day
		...
		// overtime hours
		foreach ($overtimes as $timesheet) {

			$timesheet_rel_type ='';

			$timesheet_rel_type .= 'overtime_hours';
			$column_name = $date_to_column_name[$timesheet['date_work']];

			if(isset($staff_timesheet_leave_data[$timesheet['staff_id']][$column_name])){
				$temp = $staff_timesheet_leave_data[$timesheet['staff_id']][$column_name];
				$staff_timesheet_leave_data[$timesheet['staff_id']][$column_name] = $temp.';OT:'.(float)$timesheet['value'];

			}else{
				$staff_timesheet_leave_data[$timesheet['staff_id']][$column_name] =  'OT:'.(float)$timesheet['value'];
			}

			if(isset($staff_timesheet_leave_data[$timesheet['staff_id']]['overtime_hours'])){
				$staff_timesheet_leave_data[$timesheet['staff_id']]['overtime_hours'] += (float)$timesheet['value'];
			}else{
				$staff_timesheet_leave_data[$timesheet['staff_id']]['overtime_hours'] = (float)$timesheet['value'];
			}
		...
		$results['staff_overtimes'] = $staff_overtimes;
	...
	public function timesheet_get_shifts($month)
	{
		...
		foreach ($work_shifts as $value) {
			if($value['type_shiftwork'] == 'by_absolute_time'){
				...
				// assign shifts directly to employees
				...
				// assign shifts directly to department or role
	...
	public function attendance_calculation($data)
	{
		...
		foreach ($employees_timesheets as $em_key => $timesheet) {
			...
			if(isset($employees_data[$timesheet['staff_id']])){
					...
					//check timesheet in formal contract or probationary contract.
				$payslip_month = date("m", strtotime($month));
				if(!is_null($employees_data[$timesheet['staff_id']]['probationary_expiration']) && $employees_data[$timesheet['staff_id']]['probationary_expiration'] != '0000-00-00' ){

					$probationary_expiration_month = date("m", strtotime($employees_data[$timesheet['staff_id']]['probationary_expiration'] ?? ''));
				}else{
					$probationary_expiration_month = 0;
				}
	...
	public function add_payslip_template($data){
		...
		$insert_id = $this->db->insert_id();
		hooks()->do_action('hr_payroll_after_payslip_template_added', $insert_id);
	...
	public function update_payslip_template($data, $id)
	{
		...
		if ($this->db->affected_rows() > 0) {
			...
			hooks()->do_action('hr_payroll_after_payslip_template_updated', $id);
			return true;
		}else{
			return false;
		}
	...
	public function delete_payslip_template($id){
		...
		if ($this->db->affected_rows() > 0) {
			hooks()->do_action('hr_payroll_after_payslip_template_deleted', $id);
			return true;
		}
		return false;
	}
	...
	public function delete_payslip($id)
	{
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('hr_payroll_after_payslip_deleted', $id);
			return true;
		}
		return false;
	}
	...
	{   	
		...
		$payslip_year = date('Y',strtotime($data['payslip_month'].'-01'));
		...
		foreach ($staffs as $staff_key => $staff_value) {
			...
			$staff_value['payment_run_date'] = date('d-m-Y', strtotime(date('Y-m-d')));
			$staff_value['hourly_rate'] = $staff_value['hourly_rate'];
		...
		if($arr_payslip_range[0] != $arr_payslip_range[1] && ((int)date('m', strtotime($arr_payslip_range[0])) == (int)date('m', strtotime($arr_payslip_range[1])))){
			...
			$project_salary_by_month_v2 = $this->get_project_salary_by_month($payslip_month, $str_sql, $arr_payslip_range);
		...
		foreach ($hrp_attendance as $attendance_key => $attendance_value) {
		    ...
		    if(isset($staffs_id[$attendance_value['staff_id']])){
		    	$hourly_rate = (float)$staffs_id[$attendance_value['staff_id']]['hourly_rate'];
		    	$total_workday = (float)$attendance_value['actual_workday_probation'] + (float)$attendance_value['actual_workday'] + (float)$attendance_value['paid_leave'];
		    	$basic_salary = $hourly_rate * $total_workday;
		    	$attendance_value['basic_salary'] = $basic_salary;
		...
		$_13th_month_salary = $this->get_13th_month_salary($payslip_year, $str_sql);
		foreach ($_13th_month_salary as $salary_key => $salary_value) {

			if(isset($staffs_id[$salary_value['staff_id']])){
				$staffs_id[$salary_value['staff_id']] = array_merge($staffs_id[$salary_value['staff_id']], $salary_value);
			}else{
				$staffs_id[$salary_value['staff_id']] = $attendance_value;
			}
		...
		foreach ($get_employees_data as $employee_key => $employee_value) {
			...
			$employee_value['designation'] = $employee_value['designation'];
			$employee_value['sex'] = $employee_value['sex'];
			$employee_value['birthday'] = $employee_value['birthday'];
			$employee_value['datecreated'] = $employee_value['datecreated'];
			$employee_value['nation'] = $employee_value['nation'];
			$employee_value['passport_no'] = $employee_value['passport_no'];
			$employee_value['passport_expiry'] = $employee_value['passport_expiry'];
			$employee_value['emirates_ID_No'] = $employee_value['emirates_ID_No'];
			$employee_value['EID_expiry'] = $employee_value['EID_expiry'];
			$employee_value['labour_card_No'] = $employee_value['labour_card_No'];
			$employee_value['phonenumber'] = "'".$employee_value['phonenumber'];
			$employee_value['contact_person_home_country'] = $employee_value['contact_person_home_country'];
			$employee_value['home_contact_No'] = $employee_value['home_contact_No'];
			$employee_value['other_contact_person'] = $employee_value['other_contact_person'];
			$employee_value['other_contact_No'] = $employee_value['other_contact_No'];
			$employee_value['visa_validity'] = $employee_value['visa_validity'];
    	...
    	// get project salary by month
    	$project_salary_by_month = $this->get_project_salary_by_month($payslip_month, $str_sql, $arr_payslip_range);
    	if(isset($project_salary_by_month_v2)){
    		$project_salary_by_month = $project_salary_by_month_v2;
    	}
    	foreach ($project_salary_by_month as $staff_id_key => $project_salary) {

    		if(isset($staffs_id[$staff_id_key])){
    			$staffs_id[$staff_id_key] = array_merge($staffs_id[$staff_id_key], $project_salary);
    		}else{
    			$staffs_id[$staff_id_key] = $project_salary;
    		}
    	}
				...
				// deduction_: salary deduction
					...
					if($payroll_formular != '0'){
						if(isset($taxable_salary_index) &&  $payroll_key == 'income_tax_paye'){
							...
							$pattern = '/([A-Z])([6])/'; //string replace A6;B6;C6 equal A7; A8...
							$replacement = '${1}'.($staff_row+1);
							$f = preg_replace($pattern, $replacement, $payroll_formular);
						}
		...
		if ($insert_id) {
			...
			hooks()->do_action('hr_payroll_after_payslip_added', $insert_id);
			return $insert_id;
		}
		return false;
	...
	{ 
		...
		if($this->db->affected_rows()){
			hooks()->do_action('hr_payroll_after_payslip_updated', $id);
		}
	...
	{	
		...
		if(isset($payslip_data)){
			foreach ($payslip_data as $key => $value) {
				...
				//column value
				if($value->r > 4){
					$payroll_column_value[$value->r][] = isset($value->v->v) ? ($value->v->v) : 0;
				}
			}
		}
		...
		foreach ($payslip_detail as $key => $value) {
			...
			foreach ($value as $payroll_key => $payroll_value) {
				if($payroll_key == 'payment_run_date'){
					...
					$from_format = 'd-m-Y';
					$payroll_value = _simplify_date_fix($payroll_value, $from_format);
					$payslip_detail[$key][$payroll_key]    = date('Y-m-d', strtotime($payroll_value));
				}

			    if(!in_array($payroll_key, $payroll_system_columns) && $payroll_key != 'payslip_id' && $payroll_key != 'month' ||( $payroll_key == 'bank_name' || $payroll_key == 'account_number' || $payroll_key == 'epf_no' || $payroll_key == 'social_security_no' || $payroll_key == 'designation' || $payroll_key == 'sex' || $payroll_key == 'birthday' || $payroll_key == 'datecreated' || $payroll_key == 'nation' || $payroll_key == 'passport_no' || $payroll_key == 'passport_expiry' || $payroll_key == 'emirates_ID_No' || $payroll_key == 'EID_expiry' || $payroll_key == 'labour_card_No' || $payroll_key == 'contact_person_home_country' || $payroll_key == 'home_contact_No' || $payroll_key == 'other_contact_person' || $payroll_key == 'other_contact_No' || $payroll_key == 'visa_validity' || $payroll_key == 'phonenumber' || $payroll_key == '13th_month_salary' || $payroll_key == 'hourly_rate' || $payroll_key == 'basic_salary') ){
			...
			if(isset($payslip_detail[$key]['designation'])){
				unset($payslip_detail[$key]['designation']);
			}

			if(isset($payslip_detail[$key]['sex'])){
				unset($payslip_detail[$key]['sex']);
			}
			if(isset($payslip_detail[$key]['birthday'])){
				unset($payslip_detail[$key]['birthday']);
			}
			if(isset($payslip_detail[$key]['datecreated'])){
				unset($payslip_detail[$key]['datecreated']);
			}
			if(isset($payslip_detail[$key]['nation'])){
				unset($payslip_detail[$key]['nation']);
			}
			if(isset($payslip_detail[$key]['passport_no'])){
				unset($payslip_detail[$key]['passport_no']);
			}
			if(isset($payslip_detail[$key]['passport_expiry'])){
				unset($payslip_detail[$key]['passport_expiry']);
			}
			if(isset($payslip_detail[$key]['emirates_ID_No'])){
				unset($payslip_detail[$key]['emirates_ID_No']);
			}
			if(isset($payslip_detail[$key]['EID_expiry'])){
				unset($payslip_detail[$key]['EID_expiry']);
			}
			if(isset($payslip_detail[$key]['labour_card_No'])){
				unset($payslip_detail[$key]['labour_card_No']);
			}
			if(isset($payslip_detail[$key]['contact_person_home_country'])){
				unset($payslip_detail[$key]['contact_person_home_country']);
			}
			if(isset($payslip_detail[$key]['home_contact_No'])){
				unset($payslip_detail[$key]['home_contact_No']);
			}
			if(isset($payslip_detail[$key]['other_contact_person'])){
				unset($payslip_detail[$key]['other_contact_person']);
			}
			if(isset($payslip_detail[$key]['other_contact_No'])){
				unset($payslip_detail[$key]['other_contact_No']);
			}
			if(isset($payslip_detail[$key]['visa_validity'])){
				unset($payslip_detail[$key]['visa_validity']);
			}
			if(isset($payslip_detail[$key]['phonenumber'])){
				unset($payslip_detail[$key]['phonenumber']);
			}
			if(isset($payslip_detail[$key]['13th_month_salary'])){
				unset($payslip_detail[$key]['13th_month_salary']);
			}
			if(isset($payslip_detail[$key]['hourly_rate'])){
				unset($payslip_detail[$key]['hourly_rate']);
			}
			if(isset($payslip_detail[$key]['basic_salary'])){
				unset($payslip_detail[$key]['basic_salary']);
			}
		...
		$notify_closing_payslip = get_hr_payroll_option('notify_closing_payslip');
		if($notify_closing_payslip == 1){
			$data_new = [];
			$data_new['send_payslip_to_staff'] = true;
			$this->session->set_userdata($data_new);
		}

		if ($affectedRows > 0) {
        	hooks()->do_action('after_payslip_closing', $data);
	...
	{	
	    ...
	    if($this->db->affected_rows() > 0){
        	hooks()->do_action('after_update_payslip_status', $id, $status);
	...
	{	
		...
		foreach ($es_detail as $key => $value) {
			...
			foreach ($value as $k_value => $val) {
				if($k_value != 'staff_id' && $k_value != 'id' && $k_value != 'rel_type' && $k_value != 'month' && $k_value != 'paid_leave' &&  $k_value != 'unpaid_leave' &&  $k_value != 'overtime_hours'){
					...
					if(new_strlen($val) > 0){
						...
						foreach ($val_explode as $ex_value) {
						    if(preg_match('/PL:/', $ex_value) || preg_match('/UPL:/', $ex_value) || preg_match('/OT:/', $ex_value)){
	...
	public function timesheet_leave_calculation($data)
	{
		...
		foreach ($employees_timesheets as $em_key => $timesheet_leave) {
				...
				if($k_value != 'staff_id' && $k_value != 'id' && $k_value != 'rel_type' && $k_value != 'month' && $k_value != 'paid_leave' &&  $k_value != 'unpaid_leave' &&  $k_value != 'overtime_hours'){
					...
					if(new_strlen($val) > 0){
						...
						foreach ($val_explode as $ex_value) {
							...
							}elseif(preg_match('/OT:/', $ex_value)){
								$overtime_hours += (float)str_replace('OT:', '', $ex_value);
							}
						}
					}
				}
			...
			$update_batch_data[$em_key]['overtime_hours'] = $overtime_hours;
			...
			if(count($employees_timesheets) > 0){
				...
				foreach ($employees_timesheets as $key => $value) {
					if(isset($update_batch_employees_timesheet[$value['staff_id']])){
						...
						$employees_timesheets[$key]['overtime_hours'] = $update_batch_employees_timesheet[$value['staff_id']]['overtime_hours'];
					}
				}
				if(count($employees_timesheets) > 0){
					$this->db->update_batch(db_prefix().'hrp_employees_timesheets', $employees_timesheets, 'id');
	...
	public function attendance_calculation_v2($payslip_month, $arr_payslip_range, $where = '')
	{
		...
		if(new_strlen($str_select_temp) > 0){
		...
		// get employees timesheet leaves
		...
		foreach ($employees_timesheets as $em_key => $timesheet) {
			...
			if(isset($arr_employees_timeshee_leaves[$timesheet['staff_id']])){
				...
				$overtime_hours = $arr_employees_timeshee_leaves[$timesheet['staff_id']]['overtime_hours'];
			}

			$employees_timesheets[$em_key]['paid_leave'] = $paid_leave;
			$employees_timesheets[$em_key]['unpaid_leave'] = $unpaid_leave;
			$employees_timesheets[$em_key]['overtime_hours'] = $overtime_hours;
			...
			if(isset($employees_data[$timesheet['staff_id']])){
				...
				$probationary_expiration_day = date("d", strtotime($employees_data[$timesheet['staff_id']]['probationary_expiration'] ?? ''));
					...
					//check probationary expiration month == payslip month
				foreach ($timesheet as $timesheet_key => $timesheet_value) {
					if((float)$payslip_month == (float)$probationary_expiration_month ){
			...
			if ($hrp_payslip_details) {
				...
				if ($hrp_payslip && is_numeric($hrp_payslip->pdf_template_id) && $hrp_payslip->pdf_template_id != 0) {
					...
					if ($pdf_payslip_template) {
						...
						if ($for_editor == false) {
							$this->load->library('hr_payroll/merge_fields/hr_payslip_merge_fields');
							...
							$hrp_payroll_columns = $this->get_hrp_payroll_columns();
							$logo_url = '';
							...
							foreach ($merge_fields as $key => $val) {
								...
								if($key == '{logo_image_with_url}'){
									...
									$val .= '<a href="'.$logo_url.'" class="logo hr-img-responsive" width="250">';
									$val .= '<img src="'.$logo_url.'" class="hr-img-responsive" width="250" alt="GTSS Solution Viet Nam">';
									$val .= '</a>';
							...
							foreach ($hrp_payroll_columns as $key => $value) {
								if (stripos($pdf_payslip_template->content, '{'.$value['function_name'].'}') !== false) {
									$pdf_payslip_template->content = str_ireplace('{'.$value['function_name'].'}', '', $pdf_payslip_template->content ?? '');
								}
							}
     ...
     * @return [type]        
     */
    public function get_13th_month_salary($year, $where='')
	{
	
		$this->db->select('staff_id, AVG(net_pay) as 13th_month_salary');
		if($where != ''){
			$this->db->where($where);
		}
		$this->db->where("date_format(month, '%Y') = '".$year."'");
		$this->db->group_by('staff_id');
		$_13th_month_salary = $this->db->get(db_prefix() . 'hrp_payslip_details')->result_array();

		return $_13th_month_salary;
	}

	/**
	 * send payslip to staff
	 * @param  [type]  $payslip          
	 * @param  [type]  $payslip_detail   
	 * @param  boolean $attachpdf        
	 * @param  string  $cc               
	 * @param  boolean $manually         
	 * @param  boolean $has_pdf_template 
	 * @return [type]                    
	 */
	public function send_payslip_to_staff($payslip, $payslip_detail, $attachpdf = true, $cc = '', $manually = false, $has_pdf_template = false)
    {

        $template_name = 'payslip_closing_notification_send_to_staff';
       	$payslip_detail_number = '';
        $emails_sent = [];
        $send_to = $this->input->post('sent_to_staff');
        $month_created = _d($payslip->payslip_month);
        if(!is_null($payslip->payslip_range) && $payslip->payslip_range != ''){
        	$month_created .= ' ('.$payslip->payslip_range.')';
        }

        if($manually == true){
            $send_to = [];
            $send_to[] = $payslip_detail['staff_id'];
        }
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            if ($attachpdf) {
                ...
                if(!is_null($payslip_detail['employee_number']) && $payslip_detail['employee_number'] != ''  && $payslip_detail['employee_number'] != 0 ){
					$employee_name = slug_it($payslip_detail['employee_number'] ?? '', ['separator' => '_']);
				}else{
					$employee_name = slug_it($payslip_detail['employee_name'] ?? '', ['separator' => '_']);
				}
				$payslip_detail_number = $employee_name .'_'.date('m-Y', strtotime($payslip_detail['month']));

                if($has_pdf_template){
					$_pdf_payslip = $this->hr_payroll_get_payslip_pdf_only_for_pdf($payslip_detail['id']);
					set_mailing_constant();
					$pdf = hr_payroll_payslip_pdf($_pdf_payslip);
					$attach = $pdf->Output($employee_name .'_'.date('m-Y', strtotime($payslip_detail['month'])) . '.pdf', 'S');
				}else{
					$data = [];
					$data['payslip_detail'] = $payslip_detail;

					$arr_department = $this->get_staff_departments($payslip_detail['staff_id'], true);
					$list_department = '';
					...
					$_pdf_payslip = $this->load->view('hr_payroll/employee_payslip/export_employee_payslip', $data, true);
					$_pdf_payslip .= '<link href="' . module_dir_url(HR_PAYROLL_MODULE_NAME, 'assets/css/export_employee_pdf.css') . '"  rel="stylesheet" type="text/css" />';


					set_mailing_constant();
					$pdf = $this->employee_export_pdf($_pdf_payslip);
					$attach = $pdf->Output($employee_name .'_'.date('m-Y', strtotime($payslip_detail['month'])) . '.pdf', 'S');
				}
            }

            foreach ($send_to as $staffid) {
                if ($staffid != '') {
                    ...
                    $data_send_mail = [];
                    $data_send_mail['month_created'] = $month_created;
                    $data_send_mail['firstname'] = $staff->firstname;
                    $data_send_mail['lastname'] = $staff->lastname;
                    $data_send_mail['id'] = $payslip_detail['id'];

                    $template = mail_template($template_name, 'hr_payroll', array_to_object($data_send_mail), $staff, $cc);

                    if ($attachpdf) {
                        $hook = hooks()->apply_filters('send_payslip_to_staff_file_name', [
                            'file_name' => str_replace('/', '-', $payslip_detail_number . '.pdf'),
                            'payslip'  => $_pdf_payslip,
                        ]);
                    ...
                    if ($template->send()) {
                        array_push($emails_sent, $staff->email);
                    }
                }
                $i++;
            }
        ...
        if (count($emails_sent) > 0) {
            hooks()->do_action('payslip_sent', $payslip_detail['id']);
     ...
     * @param [type] $data 
     */
    public function add_assign_worker($data)
    {
    	if (isset($data['staff_id'])) {
    		$staff_ids = $data['staff_id'];
    		unset($data['staff_id']);
    	}
    	$data['date_assign'] = to_sql_date($data['date_assign']);
    	$data['active'] = 1;
    	$data['date_created'] = date('Y-m-d H:i:s');
    	$data['createdby'] = get_staff_user_id();
    	if(isset($data['is_bonus']) && $data['is_bonus'] == 1){
    		$data['hours'] = 0;
    		$data['hourly_rate'] = 0;
    	}else{
    		$data['bonus_amount'] = 0;
    	...
    	$this->db->insert(db_prefix() . 'hrp_project_costs', $data);
    	$insert_id = $this->db->insert_id();
    	if ($insert_id) {
    		if(isset($staff_ids)){
    			foreach ($staff_ids as $key => $item) {
    				$project_for_staff = [];
    				$project_for_staff['project_cost_id'] = $insert_id;
    				$project_for_staff['staff_id'] = $item;
    				$this->db->insert(db_prefix() . 'hrp_project_for_staffs', $project_for_staff);
    			}
    		}

    		hooks()->do_action('after_hr_payroll_assign_worker_added', $insert_id);
    		return $insert_id;
    	}
    	return false;
    }
     ...
     * @return [type]       
     */
    public function update_assign_worker($data, $id)
    {
    	$affectedRows = 0;
    	if (isset($data['staff_id'])) {
    		$staff_ids = $data['staff_id'];
    	...
    	$this->db->where('id', $id);
    	$this->db->update(db_prefix() . 'hrp_project_costs', $data);
    	if ($this->db->affected_rows() > 0) {
    		$affectedRows++;
    		hooks()->do_action('after_hr_payroll_assign_worker_updated', $id);
    	}

    	$project_cost_for_staffs = $this->get_project_cost_for_staff(false, 'project_cost_id = '.$id);
        if (sizeof($project_cost_for_staffs) > 0) {
            if (!isset($staff_ids)) {
                $this->db->where('project_cost_id', $id);
                $this->db->delete(db_prefix() . 'hrp_project_for_staffs');
            } else {
                foreach ($project_cost_for_staffs as $cost_for_staffs) {
                    if (isset($staff_ids)) {
                        if (!in_array($cost_for_staffs['staff_id'], $staff_ids)) {
                            $this->db->where('project_cost_id', $id);
                            $this->db->where('staff_id', $cost_for_staffs['staff_id']);
                            $this->db->delete(db_prefix() . 'hrp_project_for_staffs');
                            if ($this->db->affected_rows() > 0) {
                                $affectedRows++;
                            }
            ...
            if (isset($staff_ids)) {
                foreach ($staff_ids as $staff_id) {
                    ...
                    $_exists = $this->db->get(db_prefix() . 'hrp_project_for_staffs')->row();
                    if (!$_exists) {
                        $this->db->insert(db_prefix() . 'hrp_project_for_staffs', [
                            'project_cost_id'      => $id,
                            'staff_id' => $staff_id,
                        ]);
                        if ($this->db->affected_rows() > 0) {
     ...
     * @return [type]         
     */
    public function get_assign_worker($id = false, $where = []) {

    	if (is_numeric($id)) {
    		$this->db->where('id', $id);
    		$project_cost = $this->db->get(db_prefix() . 'hrp_project_costs')->row();

    		$this->db->where('project_cost_id', $id);
    		$this->db->order_by('id', 'ASC');
    		$project_for_staffs = $this->db->get(db_prefix().'hrp_project_for_staffs')->result_array();
    		if($project_for_staffs){
    			$project_cost->project_for_staffs = $project_for_staffs;
    		}
    		return $project_cost;
    	}
    	...
    	if ($id == false) {
    		...
    		return $this->db->get(db_prefix() . 'hrp_project_costs')->result_array();
    	}
    }

    /**
     * get project cost for staff
     * @param  boolean $id    
     * @param  array   $where 
     * @return [type]         
     */
    public function get_project_cost_for_staff($id = false, $where = []) {

        if (is_numeric($id)) {
            $this->db->where('id', $id);
            $project_for_staff = $this->db->get(db_prefix() . 'hrp_project_for_staffs')->row();
            
            return $project_for_staff;
        }
        if ($id == false) {
            if ((is_array($where) && count($where) > 0) || (is_string($where) && $where != '')) {
                $this->db->where($where);
            ...
            return $this->db->get(db_prefix() . 'hrp_project_for_staffs')->result_array();
        }
    }

    /**
     * delete assign worker
     * @param  [type] $id 
     * @return [type]     
     */
    public function delete_assign_worker($id)
    {
    	hooks()->do_action('before_hr_payroll_assign_worker_deleted', $id);

    	$affected_rows = 0;

    	$this->db->where('id', $id);
    	$this->db->delete(db_prefix() . 'hrp_project_costs');
    	if ($this->db->affected_rows() > 0) {
    		hooks()->do_action('after_hr_payroll_assign_worker_deleted', $id);
     ...
     * @param  [type] $month         
     * @param  [type] $staff_id      
     * @param  array  $payslip_range 
     * @return [type]                
     */
    public function get_project_salary_by_month($month, $staff_id, $payslip_range = [])
	{
		$month = date('Y-m', strtotime($month));
		$arr_project_salary=[];
		...
		if(count($payslip_range) == 0){
			$sql_where = 'SELECT 
			DATE_FORMAT(ps.date_assign, "%Y-%m") AS month,
			SUM(ps.hours * ps.hourly_rate) AS _project_salary,
			SUM(ps.bonus_amount) AS _bonus_amount,
			ps.staff_id
			FROM (
				SELECT date_assign, hours, hourly_rate, bonus_amount, staff_id
				FROM '.db_prefix().'hrp_project_costs
				LEFT JOIN '.db_prefix().'hrp_project_for_staffs ON '.db_prefix().'hrp_project_costs.id = '.db_prefix().'hrp_project_for_staffs.project_cost_id
					WHERE DATE_FORMAT('.db_prefix().'hrp_project_costs.date_assign, "%Y-%m") = "'.$month.'" AND '.$staff_id.' 
			) as ps
			GROUP BY DATE_FORMAT(ps.date_assign, "%Y-%m"), ps.staff_id
			ORDER BY month';
			...
			FROM (
				...
				FROM '.db_prefix().'hrp_project_costs
				LEFT JOIN '.db_prefix().'hrp_project_for_staffs ON '.db_prefix().'hrp_project_costs.id = '.db_prefix().'hrp_project_for_staffs.project_cost_id
					WHERE DATE_FORMAT('.db_prefix().'hrp_project_costs.date_assign, "%Y-%m-%d") >= "'.$payslip_range[0].'" AND DATE_FORMAT('.db_prefix().'hrp_project_costs.date_assign, "%Y-%m-%d") <= "'.$payslip_range[1].'" AND '.$staff_id.' 
			) as ps
			GROUP BY DATE_FORMAT(ps.date_assign, "%Y-%m"), ps.staff_id
			ORDER BY month';
		}

		$project_salaries = $this->db->query($sql_where)->result_array();

		foreach ($project_salaries as $key => $project_salary) {
			if(isset($arr_project_salary[$project_salary['staff_id']])){
				$arr_project_salary[$project_salary['staff_id']]['project_salary'] += (float)$project_salary['_project_salary'];
				$arr_project_salary[$project_salary['staff_id']]['project_bonus'] += (float)$project_salary['_bonus_amount'];
			}else{
				$arr_project_salary[$project_salary['staff_id']]['project_salary'] = (float)$project_salary['_project_salary'];
				$arr_project_salary[$project_salary['staff_id']]['project_bonus'] = (float)$project_salary['_bonus_amount'];
			}
	 ...
	 * @return [type] 
	 */
	public function process_export_jobs()
	{
		$jobs = $this->db->where('status', 'processing')->limit(5)->get('hrp_export_jobs')->result();
		foreach ($jobs as $job) {
			
			try {
            // Xuất PDF payslip
				$file_path = $this->export_payslip_task($job->payslip_id);

				$this->db->where('id', $job->id)->update('hrp_export_jobs', [
					'status' => 'done',
					'file_path' => $file_path,
	 ...
	 * @return [type]     
	 */
	public function export_payslip_task($id)
	{
		...
		// check continue creating file if last cron did not finish creating file
		$file_created = false;
		$arr_payslip_file = [];

		$_payslip = $this->hr_payroll_model->get_hrp_payslip($id);
		$payslip_details = $this->hr_payroll_model->get_payslip_detail_by_payslip_id($id);

		$zip_file = HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP .$id.'/'. $_payslip->payslip_name . '_' . date('m-Y', strtotime($_payslip->payslip_month)) . '.zip';
		...
		foreach(glob(HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP .$id.'/'. '*') as $file) { 
			$file_arr = new_explode("/",$file);
			$filename = array_pop($file_arr);

			if(!$file_created){
				$arr_payslip_file[] = $filename;
			}
			...
			if($file_created){
				...
				if(file_exists($file)) {
					if ($filename != 'index.html') {
						unlink(HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP.$id.'/'.$filename);
					}
				}
			}
		}
		...
		$check_payslip_has_pdf_template = check_payslip_has_pdf_template($id);
		...
		foreach ($payslip_details as $payslip_detail) {
			...
			if(in_array($employee_name .'_'.date('m-Y', strtotime($payslip_detail['month'])) . '.pdf', $arr_payslip_file)){
				continue;
			}
			...
			$this->re_save_to_dir($pdf, $employee_name .'_'.date('m-Y', strtotime($payslip_detail['month'])) . '.pdf', $id);
		}
		...
		foreach(glob(HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP .$id.'/'. '*') as $file) {
			...
			$this->zip->read_file(HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP.$id.'/'. $filename);
		}

	    $this->zip->archive($zip_file); // Lưu file zip
	    return $zip_file;
	}
	 ...
	 * @return [type]            
	 */
	public function re_save_to_dir($pdf, $file_name, $id)
	{
		$dir = HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP.$id.'/';
