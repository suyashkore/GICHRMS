...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    private function calcSheetOffsets()
    {
        ...
        foreach ($this->phpExcel->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + new_strlen(PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
        ...
        if (count($this->phpExcel->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                for ($i = 0; $i < count($range); $i++) {
                    $range[$i][0] = '\'' . new_str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . PHPExcel_Cell::absoluteCoordinate($range[$i][0]);
                    if (isset($range[$i][1])) {
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $length = 2 + new_strlen($numberFormatString);    // Number of bytes to follow
    ...
    {
        $record = 0x0017;                     // Record identifier
        $length = 0x02 + new_strlen($sheetname);  // Number of bytes to follow

        $cch    = new_strlen($sheetname);         // Length of sheet name
    ...
    private function writeSharedStringsTable()
    {
        ...
        foreach (array_keys($this->stringTable) as $string) {
            ...
            while ($finished === false) {
                ...
                if (new_strlen($recordData) + new_strlen($string) <= $continue_limit) {
                    ...
                    $space_remaining = $continue_limit - new_strlen($recordData);
                    ...
                    // 2. space remaining is greater than or equal to minimum space needed
                        ...
                        if ($encoding == 1 && (new_strlen($string) - $space_remaining) % 2 == 1) {
        ...
        foreach ($recordDatas as $i => $recordData) {
            ...
            $header = pack("vv", $record, new_strlen($recordData));
