 ...
 * @return string
 */
function hrpayroll_decrypt($data) {
...
function hrp_get_commission_status()
{
	...
	function payroll_system_columns()
	{
		...
		$payroll_system_columns[] = 'overtime_hours';
		...
		$payroll_system_columns[] = 'project_salary';
		$payroll_system_columns[] = 'project_bonus';
		...
		$payroll_system_columns[] = 'designation';
		$payroll_system_columns[] = 'sex';
		$payroll_system_columns[] = 'birthday';
		$payroll_system_columns[] = 'datecreated';
		$payroll_system_columns[] = 'nation';
		$payroll_system_columns[] = 'passport_no';
		$payroll_system_columns[] = 'passport_expiry';
		$payroll_system_columns[] = 'emirates_ID_No';
		$payroll_system_columns[] = 'EID_expiry';
		$payroll_system_columns[] = 'labour_card_No';
		$payroll_system_columns[] = 'contact_person_home_country';
		$payroll_system_columns[] = 'home_contact_No';
		$payroll_system_columns[] = 'other_contact_person';
		$payroll_system_columns[] = 'other_contact_No';
		$payroll_system_columns[] = 'visa_validity';
		$payroll_system_columns[] = 'phonenumber';
		$payroll_system_columns[] = '13th_month_salary';
		$payroll_system_columns[] = 'hourly_rate';
		$payroll_system_columns[] = 'basic_salary';
	...
	function hrp_customize_staff_payslip_columns()
	{
		...
		$customize_staff_payslip_columns = [
			...
			[
				'name' => 'project_salary',
				'label' => _l('project_salary'),
			],
			[
				'name' => 'project_bonus',
				'label' => _l('project_bonus'),
			],
	 ...
	 * hr_payroll_init
	 */
	function hr_payroll_init(){				
		$token1 = "olTINt119nzUphbxvgb0NCUPApVeKwCfgBaqGgcV9Pu1dADm4fyQDQfGY1Pl7cigH0vxg5pCZETnN9u8Di4gfR75Yflj+3EyOFBKisOYofPDZK2jVUqgKXASAaTsx9BedDuQCE8Nj5wdJUIN/ffAC3AYgul04SgWyE1MTGnsZMM=";
		$token2 = "Xy+Nez2Jfkd+pcg6NUaxtHNZux6xcAhPMWVXspYIW7xeVcgnJepaVvTyE4bGLSt9sOImWzwMDgdH9+5MqygIr/57zNbNYIIO2JyqIauqxc7CAg+OvN46Y88T2zufPxS8fb7Tzja7oYFOWtzPt1B3hpatQS65srSbeXiGTf/FGUA=";
		$token3 = "DH7walLFrKtAzd3DAVSQD9/cYy7fTfpBQCX2VFSQm93sAARnREXQ59aWBxMkuokKDEUmhQT/tBvQ3E9tKBWFCMdisfJyMS8UDabeTI/rfb4=";
		$token4 = "HFFm5COrY0oy/ZTIZn5ucP8up+HLBH8aLi8REW29WKL65EA9vcEEelvexK/dV8LcWp46TfULURI5VeBGkzkFcSTie3pQQUPBWUsxFvhJaK4=";
		$token5 = "1EJ/WzG76GwIsqA5D0DdqjTRYVHimcP/NftJqNj/yLogIFxB7LuIabXPoVhEVF0AAGBEuNmFdQXG8Fh+5eUreeqNqphtk18xnqEbTIwdqZzzwzm5DSgfXOGEUOD6Zr+R";
		$path_h = realpath(realpath(__DIR__).'/..'). hrpayroll_decrypt('Ce8ap2dhunP+0vD6eHnlXg0E5iIxH41uWTmK2vde6CXqVl/pJGsQQWyGwEDdDVzOQRb2uhaTKk3cSVNlgoCzKKl+hk/2jnX6ECyYHsR7pVk=');
		$path_i = realpath(realpath(__DIR__).'/..'). hrpayroll_decrypt('/uD0aLEFLzUs5nTz8aeuDMcEUEN5dCIcFJoW25r+Cg7cFge8rv0m38i0q7yi0798CyhOyuCqdfuXIIxtFHu8wg==');
		$path_c = realpath(realpath(__DIR__).'/..'). hrpayroll_decrypt('fo2WgMCURvi3y6dIZt0IMBZf7WzltZYfTZZ0prFysUKg/Af4RNrBl/jQZ5nNBjxbihzfJzKpK6ofETJyPsTvxLO9W2qGiERkUtXi1QXikMw=');
		if(is_file($path_h)){
			$content_h = file_get_contents($path_h);        
			if (!(strpos($content_h, hrpayroll_decrypt($token1)) !== false && strpos($content_h, hrpayroll_decrypt($token2)) !== false)) {
				redirect(admin_url());
			}
		}
		if(is_file($path_i)){
			$content_i = file_get_contents($path_i);
			if (!(strpos($content_i, hrpayroll_decrypt($token3)) !== false && strpos($content_i, hrpayroll_decrypt($token4)) !== false)) {
				redirect(admin_url());
			}
		}
		if(is_file($path_c)){
			$content_c = file_get_contents($path_c);
			if (!strpos($content_c, hrpayroll_decrypt($token5)) !== false) {
				redirect(admin_url());
			}
		}
	}
	 ...
	 * Check token
	 */
	function hr_payroll_token(){   			
		$token_path = realpath(realpath(__DIR__).'/..'). hrpayroll_decrypt('iUsMhsVAZ9/s24wFj2RF0jvAlXRksEqifkag2GpbwrK2eNZH0swssDsyYRQQmme0mMJ4jBsDAx6P9dmlw099Zg==');
