<script >
	(function(){
		...
		var options = {
			...
			functionButton: '<?php echo new_html_entity_decode($permission_actions); ?>',
		...
		var data_payslip = {};
		<?php if(isset($send_payslip_to_staff)){ 
			?>
			data_payslip.id = <?php echo new_html_entity_decode($payslip->id); ?>;
			$.get(admin_url+'hr_payroll/send_payslip_to_staff', data_payslip).done(function(response){
		...
		$.ajax({ 
			...
			if(response.success == true){
				...
				$('.payslip_download').attr({target: '_blank', 
					href  : site_url +response.file_url});
			}else{
				alert_float('warning', response.message);
			}
		});
