...
define('HR_PAYROLL_REVISION', 117);
...
//get currency
hooks()->add_action('before_cron_run', 'hrp_cronjob_currency_rates');

//mail theme
hooks()->add_action('after_email_templates', 'add_hr_payroll_email_templates');
register_merge_fields('hr_payroll/merge_fields/hr_payroll_merge_fields');
hooks()->add_filter('other_merge_fields_available_for', 'hr_payroll_register_other_merge_fields');

//Project hook
hooks()->add_filter('project_tabs', 'hrp_init_project_tabs');
...
if(!(strpos($viewuri, '/admin/projects/view') === false)  && !(strpos($viewuri, '?group=assign_workers') === false)){
    echo '<script src="' . module_dir_url(HR_PAYROLL_MODULE_NAME, 'assets/js/projects/assign_worker.js') .'?v=' . HR_PAYROLL_REVISION.'"></script>';
}
...
function hr_payroll_appint(){
    ...
    if(!$hrpayroll_gtssres || ($hrpayroll_gtssres && isset($hrpayroll_gtssres['status']) && !$hrpayroll_gtssres['status'])){
        ...
        set_alert('danger', hrpayroll_decrypt('plGJBtUappDE3nHKyVHLrW7/hP1hS3GF2gj8p2o/ZLC5ETPQIs2GtJLQ1eOaoWXUBkF/F/RnkOiHDxsFxIAzAQlGrUZAfVufkpsftfXeGYKDHJWgpXR0Flf6Tw1G123yLoumNQnMbhRW2RPdhagn7A+KxbS8QEG0NzP8I0zFUG/ZG+V6oG4OTdETpxFBRQincNmA960Lj/a4aDuS1Hj2Hw=='));
...
function hrp_cronjob_currency_rates($manually) {
    ...
    $CI->hr_payroll_model->process_export_jobs();
}

if (!function_exists('add_hr_payroll_email_templates')) {
    /**
     * Init hr payroll email templates and assign languages
     * @return void
     */
    function add_hr_payroll_email_templates($email_type) {
        $CI = &get_instance();

        $data['hr_payroll_templates'] = $CI->emails_model->get(['type' => 'hr_payroll', 'language' => 'english']);
        $data['email_type'] = $email_type;
        $CI->load->view('hr_payroll/email_templates/email_templates', $data);
    }
}

function hr_payroll_register_other_merge_fields($for)
{
    $for[] = 'hr_payroll';
...
function hrp_init_project_tabs($tabs){

    $child = [];

    if (hr_payroll_get_status_modules('hr_payroll')) {
        
            $child[] = [
                'parent_slug' => 'hr_payroll',
                'slug' => 'assign_workers',
                'name' => _l('hrp_assign_workers'),
                'view' => 'hr_payroll/projects/assign_worker',
                'position' => 5,
                'visible' => true,
                'icon' => '',
                'href' => '#',
