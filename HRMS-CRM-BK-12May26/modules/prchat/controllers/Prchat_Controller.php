...
class Prchat_Controller extends AdminController
{
    ...
    public function addNewChatGroupMembersModal()
    {
        ...
        $users = $this->chat_model->getUsers();
        $clients = $this->chat_model->getClient();
        $data['staff'] = [];
        $data['client'] = [];
        $group_id = $this->input->get('group_id');
        $currentUsers = $this->getCurrentGroupUsers($group_id);
        $currentClients = $this->getCurrentGroupClients($group_id);
        ...
        foreach ($clients as $selector => $client) {
            foreach ($currentClients as $currentClient) {
                if ($currentClient['client_id'] == $client['userid']) {
                    unset($clients[$selector]);
                }
            }
        }
    ...
    public function addChatGroupMembers()
    {
        ...
        if (!empty($this->input->post('group_name'))) {
            ...
            $members = $this->input->post('members');
            $group_id = $this->input->post('group_id');
            $client_members = $this->input->post('client_members');

            return $this->chat_model->addChatGroupMembers($group_name, $group_id, $members, $client_members,$this->pusher);
        }
    }
    ...
    public function getCurrentGroupClients()
    {
        if (!$this->input->is_ajax_request()) {
            redirect('admin/prchat/Prchat_Controller/chat_full_view', 'refresh');
        }
        ...
        if ($this->input->post('group_id') !== '') {
            $group_id = $this->input->post('group_id');
            $users = $this->chat_model->getCurrentGroupClients($group_id);
            if (is_array($users) && !empty($users)) {
                return $users;
            } else {
                return false;
