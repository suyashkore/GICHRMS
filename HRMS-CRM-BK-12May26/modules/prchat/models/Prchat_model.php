...
class Prchat_model extends App_Model
{
    ...
    public function getClient()
    {
        ...
        $this->db->select('userid, firstname, lastname, email, phonenumber');
        $this->db->where('active', 1);
        $Client = $this->db->get(db_prefix() . 'contacts')->result_array();
        ...
        if ($group_id !== null) {
            ...
            foreach ($query as &$chat) {
                ...
                if($chat->sender_id){
                    ...
                    $chat->sender_fullname = get_client_contact_full_name($chat->client_id);
                    $chat->user_image = $this->getClientImage($chat->client_id);
                }
     ...
     * @param instance $pusher
     */
    public function addChatGroupMembers($group_name, $group_id, $members,$client_members, $pusher)
    {
        $member_ids = [];
        ...
        foreach ($client_members as $client_member_id) {
            $this->db->where('group_name', $group_name);
            $this->db->where('group_id', $group_id);
            $this->db->insert(TABLE_CHATGROUPMEMBERS, ['group_name' => $group_name, 'client_id' => $client_member_id, 'group_id' => $group_id]);
            array_push($member_ids, $member_id);
        }
    ...
    public function getCurrentGroupClients($group_id)
    {
        $client = '';

        $group_id = $this->input->post('group_id');
        $this->db->select('client_id, lastname, firstname');
        $this->db->from(TABLE_CHATGROUPMEMBERS);
        $this->db->where('group_id', $group_id);
        $this->db->join(db_prefix() . 'contacts', db_prefix() . 'contacts.userid = ' . TABLE_CHATGROUPMEMBERS . '.client_id');
        $query = $this->db->get();
