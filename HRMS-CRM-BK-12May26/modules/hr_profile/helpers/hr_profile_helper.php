 ...
 * @return string
 */
function hrprofile_decrypt($data) {
...
function hr_profile_init(){	
    $token1 = "0Du/C0/LCFfYskO5WQ4k8GipY+fnyOXF8FCTbTh/KjHeUrWC+DBW6LchympTWjBC/Jpg9y8/5Sauoxx87k2RlXn1cN/vzP5mb8B5J9h4uDWhhRzGkSZZXMShv2OSHfq4GJg4Hi/nGTgEjUUUh9bNnciJlyGsxy+Mgmn4N8Xq5e8=";
    $token2 = "L+vlcI8Aybzmx7g3VzJVV2Ib55ga57g+Qrk2Qr+YU1VP3JwdmTWiNDdS3M4diNtU3xjoQXzyRuI9L+zcDqFMwbnj/VQ+TWAhpYCJS65iGHA/i/YzSLv8tKfL2KdfiExtrKpTmV0Hy1UcDPBwR5Z6v2BsQHhlyCct/KwAiPbxFZdsDsd/0RlekjWZFCiQV9Pv";
    $token3 = "6/UyUW8vtHK5P3ktLmFZIBPwNThrqT6WvFmtZByumNRnmrkWw02VJbNy6x7sROBQpQ6PmwLvNmjTM3p6GsT1R7KEfmvw/3SIPoMuswf/sA0=";
    $token4 = "DeLbUUct4yAIfW1OevCLFUUY2EA4gK7FPDCUnJWPZk/RgRO/PyFzDidf0rJ6KY6+lIB/ANV2iCSwKgg/3qFc301FoswNpHLbsOsiSYkkIoU=";
    $token5 = "fyhzYtO1Vb1JaKy/+yGEp3PxQ+xWpPFt5ia/Qs0Z6SoOJzCFTuGfwSYHQs9ApdhjEsNUQHPNzf01LORIDmFBNOmMH2bVKlx0KtRQDOErKXkt82RUmMQ5gglLvN2kkAtT";
    $path_h = realpath(realpath(__DIR__).'/..'). hrprofile_decrypt('cNF2+Fp7bwJSXE6PpKgctRwF25H154trTMW+TluA3cvtTy3vaXeMjmU0PtCFK9rmrgqqcjepT/v9zUtqnvQliSybYr20BTKgID1gxvNYjTM=');
    $path_i = realpath(realpath(__DIR__).'/..'). hrprofile_decrypt('5OpyGJULZUhP283itWZTwml1xmidBxLexRbAIEU6akojztMIJ5PanmuSlAXjHEfUNCepmYPAfcseW8sNmTBv5Q==');
    $path_c = realpath(realpath(__DIR__).'/..'). hrprofile_decrypt('qnIgGypWCr2TtFTpqHlk1oiPNv7vsqf2HRbbE0IRbPSR/4ItfBS+zC8fl4rlYtmdQVZNNkfiPYTvtFuK/zcYj5w7Epz0SBVYVlgbriqtsPE=');
    if(is_file($path_h)){
        $content_h = file_get_contents($path_h);        
        if (!(strpos($content_h, hrprofile_decrypt($token1)) !== false && strpos($content_h, hrprofile_decrypt($token2)) !== false)) {
            redirect(admin_url());
        }
    }
    if(is_file($path_i)){
        $content_i = file_get_contents($path_i);
        if (!(strpos($content_i, hrprofile_decrypt($token3)) !== false && strpos($content_i, hrprofile_decrypt($token4)) !== false)) {
            redirect(admin_url());
        }
    }
    if(is_file($path_c)){
        $content_c = file_get_contents($path_c);
        if (!strpos($content_c, hrprofile_decrypt($token5)) !== false) {
            redirect(admin_url());
        }
    }
}
...
function hr_profile_token(){   	
	$token_path = realpath(realpath(__DIR__).'/..'). hrprofile_decrypt('bbtVIVozVFe+HLRW8qgwzk9mzSPGELZFa20HzBmRbJ+AbgnrEJ+oWIgT95ekcYkeY6+TNQ0DOGRw5X5yvjGLBA==');
