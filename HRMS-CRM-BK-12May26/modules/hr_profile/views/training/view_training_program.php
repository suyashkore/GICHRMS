			...
			<div class="col-md-12">
				...
				<div class="panel_s">
					...
					<div class="panel-body">
						<h4 class="no-margin">
							<?php echo new_html_entity_decode($training_program->training_name); ?>
						</h4>
						...
						<div class="row">
								...
								<table class="table border table-striped table-margintop">
									<tbody>
										...
										<tr class="project-overview">
											...
											<td><?php echo new_html_entity_decode(get_type_of_training_by_id($training_program->training_type)) ; ?></td>
										</tr>
										<tr class="project-overview">
										...
										<tr class="project-overview">
											...
											<td><?php echo new_html_entity_decode($training_program->mint_point) ; ?></td>
										</tr>
							...
							<div class="col-md-12">
								...
								<hr class="hr-color">
								<h5><?php echo new_html_entity_decode($training_program->description) ; ?></h5>

							</div>
							<div class="col-md-12">
								<hr class="hr-color">
							</div>
							<?php echo html_entity_decode($htmlfile); ?>
									...
									<?php foreach ($training_results as $key => $value) { ?>
										...
										<tr>
											...
											<td>
												...
												echo _l('total_point').' / '._l('hr_mint_point') .': '.new_html_entity_decode(isset($value['training_program_point']) ? $value['training_program_point'] : '') .'/'.new_html_entity_decode($training_program->mint_point) ;
												?>
											</td>
											<td></td>
										...
										<?php if(isset($value['staff_training_result'])){ ?>
											<?php  foreach ($value['staff_training_result'] as $r_key => $r_value) { ?>
												<tr>
													...
													<td>
														<a href="<?php echo admin_url('hr_profile/participate/view_staff_training_result/'.$r_value['staff_id'].'/'.$r_value['resultsetid'].'/'.$r_value['training_id'].'/'.$r_value['hash']); ?>"><?php echo '&nbsp;&nbsp;&nbsp;+'. new_html_entity_decode($r_value['training_name']); ?></a>
													...
													<td><?php echo new_html_entity_decode(_dt($r_value['date'])) ?></td>
													<td></td>
