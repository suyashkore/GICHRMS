						...
						<div role="tabpanel" class="tab-pane" id="job_position_tab">
							<div class="_buttons">
								<?php if (is_admin() || has_permission('staffmanage_job_position', '', 'create')) {?>
									<a href="#" onclick="hrrecord_new_job_position(); return false;" class="btn btn-primary pull-left ">
										<?php echo _l('hr_new_job_position'); ?>
									</a>
								<?php }?>

									<div class="btn-group mleft5">
										<a href="#" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo _l('hr_position_groups') . ' '; ?><span class="caret"></span></a>
								...
								<div class="form-group ">
									<select name="job_p_id[]" class="selectpicker" id="job_p_id" data-width="100%" data-live-search="true" multiple="true" data-actions-box="true" data-none-selected-text="<?php echo _l('hr_job_p'); ?>">
										<?php foreach ($job_p_id as $p) {?>
											<option value="<?php echo new_html_entity_decode($p['job_id']); ?>"><?php echo new_html_entity_decode($p['job_name']); ?></option>
										<?php }?>
									</select>
								</div>
							</div>
