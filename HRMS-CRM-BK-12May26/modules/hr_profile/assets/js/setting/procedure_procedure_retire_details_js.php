<script>       
	...
	function add_procedure_retire(){
		...
		var html = '';
		html += '<div class="row">';
		html += '<input type="hidden" name="procedure_retire_id" value="<?php echo new_html_entity_decode($id);?>">';
		...
		html += '<?php foreach ($staffs as $s) { ?><option value="<?php echo new_html_entity_decode($s['staffid']); ?>" ><?php echo get_staff_full_name($s['staffid']); ?></option><?php } ?>';
		html += '</select></div>';
		html += '</div>';
		...
		$('#add_save').remove();
		$('._buttons').append('<button type="submit" class="btn btn-primary pull-left mleft10"><?php echo _l('submit'); ?></button>');
		init_selectpicker();
	}
