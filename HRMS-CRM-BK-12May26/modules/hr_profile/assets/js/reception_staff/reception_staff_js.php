	<script>	
		...
		var count_id = '<?php echo new_html_entity_decode($count_id); ?>';
		...
		function save_add_asset(){
			...
			data.name = rtrim(t);
			$.post(admin_url+'hr_profile/add_new_asset/<?php echo new_html_entity_decode($staff->staffid); ?>',data).done(function(response){
				response = JSON.parse(response);
				if(response.success == true) {
		...
		function delete_asset(el){
			'use strict';
			var val = $(el).data('id');
			$.post(admin_url+'hr_profile/delete_asset/'+val+'/<?php echo new_html_entity_decode($staff->staffid); ?>').done(function(response){
				response = JSON.parse(response);
				if(response.success == true) {
					$('#asset_list').html(response.data);
