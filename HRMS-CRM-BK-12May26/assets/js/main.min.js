...
function clear_item_preview_values(e) {
    var t = $("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),
        a = $(".main");
    a.find("textarea").val(""), a.find('td.custom_field input[type="checkbox"]').prop("checked", !1), a.find("td.custom_field input:not(:checkbox):not(:hidden)").val(""), a.find("td.custom_field select").selectpicker("val", ""), a.find('input[name="quantity"]').val(1), a.find("select.hsn_code").selectpicker("val", ''), a.find("select.tax").selectpicker("val", t), a.find('input[name="rate"]').val(""), a.find('input[name="unit"]').val(""), $('input[name="task_id"]').val(""), $('input[name="expense_id"]').val("")
}
...
function add_item_to_table(e, t, a, i) {
    if ("" !== (e = void 0 === e || "undefined" == e ? get_item_preview_values() : e).description || "" !== e.long_description || "" !== e.rate) {
        ...
        return get_taxes_dropdown_template(l, e.taxname).done((function(a) {
            hsnhtml = '<div class="form-group select-placeholder">';
            hsnhtml += '<select name="newitems[' + s + '][hsn_code]" class="selectpicker" data-width="100%" data-live-search = "true"data-none-selected-text="None selected">';
            hsnhtml += '<option value =""></option>';
            hsnhtml += '<option value="998313" '; 
            if(e.hsncode == "998313"){ var sel = "selected"; }
            hsnhtml += ' '+sel+'>998313</option>';
            if(e.hsncode == "998314"){ var sel2 = "selected"; }
            hsnhtml += '<option value="998314" '+sel2+'>998314</option>';
            if(e.hsncode == "998361"){ var sel3 = "selected"; }
            hsnhtml += '<option value="998361" '+sel3+'>998361</option>';
            hsnhtml += '</select>';
            hsnhtml += '</div>';
...
function get_item_preview_values() {
    var e = {};
    return e.description = $('.main textarea[name="description"]').val(), e.long_description = $('.main textarea[name="long_description"]').val(), e.hsncode = $(".main select.hsn_code").selectpicker("val"), e.qty = $('.main input[name="quantity"]').val(), e.taxname = $(".main select.tax").selectpicker("val"), e.rate = $('.main input[name="rate"]').val(), e.unit = $('.main input[name="unit"]').val(), e
}
