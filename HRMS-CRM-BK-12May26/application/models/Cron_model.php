...
class Cron_model extends App_Model
{
    ...
    public function StopTaskTimer($manually = false)
    {   
        //$this->db->set('end_time', "UNIX_TIMESTAMP(DATE_ADD(from_unixtime(start_time), INTERVAL $older_than_hours HOUR))", false);
        $this->db->set('end_time', "UNIX_TIMESTAMP()",false);
        $this->db->where('end_time IS NULL');
        $this->db->where('task_id !=', '0');
        //$this->db->where('staff_id', 19); // Ajinkya Sir staff_id
        $this->db->update(db_prefix() . 'taskstimers');
    }
    
    public function NotifyAfter2HoursTaskStartTimer()
    {
        $older_than_hours = 2;
        ...
        //$this->db->where('staff_id', 19); // Ajinkya Sir staff_id
        $StaffList = $this->db->get(db_prefix() . 'taskstimers')->result_array();
        $eventNotifications = hooks()->apply_filters('event_notifications', true);
        foreach($StaffList as $key=>$val){
            if ($eventNotifications) {
                $additional_data = "Task timer alert!";
                $notified = add_notification([
                    'description'     => 'Your task timer has been running for over 2 hours.',
                    'touserid'        => $val['staff_id'],
                    'fromcompany'     => true,
                    'link'            => '#taskid=' . $val['task_id'],
                    'additional_data' => serialize([
                        $additional_data,
                    ]),
                ]);
                array_push($notificationNotifiedUsers, $val['staff_id']);
            }
        }
