...
class Import_business_data extends App_import
{ 
    ...
    public function perform()
    {
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            for ($i = 0; $i < $totalDatabaseFields; $i++) {
                ...
                if ($databaseFields[$i] == 'status') {
                    
                    $row[$i] = 1;
                    
                } elseif ($databaseFields[$i] == 'updated_data') {
                    $row[$i] = date('Y-m-d');
                } elseif ($databaseFields[$i] == 'created_date') {
                    $row[$i] = date('Y-m-d');
                } /*elseif ($databaseFields[$i] == 'effective_date') {
                    $row[$i] = $effective_date;
...
*/
                ...
                if (!$this->isSimulation()) {
                    $this->ci->db->insert(db_prefix().'business_data', $insert);
