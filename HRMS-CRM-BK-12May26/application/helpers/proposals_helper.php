...
function GetContactListCount($rel_id)
{
    $CI = &get_instance();
    $CI->db->select('COUNT(id) AS TotalContacts');
    $CI->db->where('userid', $rel_id);
    $CI->db->where('active', 1);
    $ContactCounts = $CI->db->get(db_prefix() . 'contacts')->row();
    return $ContactCounts->TotalContacts;
}

function GetClientContactListCount($rel_id)
{
    $CI = &get_instance();
    $CI->db->select('tblcontacts.*');
    $CI->db->where('userid', $rel_id);
    $CI->db->where('active', 1);
    $ContactCounts = $CI->db->get(db_prefix() . 'contacts')->result_array();
    return $ContactCounts;
}

function GetleadContactListCount($rel_id)
{
    $CI = &get_instance();
    $CI->db->select('tblleads.name AS firstname,tblleads.email,tblleads.phonenumber');
    $CI->db->where('id', $rel_id);
    $ContactCounts = $CI->db->get(db_prefix() . 'leads')->result_array();
    return $ContactCounts;
}

function GetProposalAccenptCount($id)
{
    $CI = &get_instance();
    $CI->db->select('COUNT(id) AS TotalAccepts');
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', "proposal");
    $AcceptsCounts = $CI->db->get(db_prefix() . 'signiture_details')->row();
    return $AcceptsCounts->TotalAccepts;
}

function GetContractAcceptList($id)
{
    $CI = &get_instance();
    $CI->db->select('tblsigniture_details.*');
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', "proposal");
    $AcceptsList = $CI->db->get(db_prefix() . 'signiture_details')->result_array();
    return $AcceptsList;
}
