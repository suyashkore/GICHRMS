...
class Proposal extends ClientsController
{
    public function index($id = '', $hash = '')
    {
        ...
        $proposalAcceptList = $this->proposals_model->GetAcceptList($id);
        if ($proposal->rel_type == 'customer') {
            $ContactsList = GetClientContactListCount($proposal->rel_id);
        } else if($proposal->rel_type == 'lead') {
            $ContactsList = GetleadContactListCount($proposal->rel_id);
        }
        ...
        if ($this->input->post()) {
            ...
            switch ($action) {
                ...
                case 'accept_proposal':
                        ...
                        $data["rel_id"] = $id;
                        $data["rel_type"] = "proposal";
                        if($this->db->insert(db_prefix().'signiture_details', $data)){
                            if ($proposal->rel_type == 'customer' && !is_client_logged_in()) {
                                $ProAcceptCount = GetProposalAccenptCount($id);
                                $ContactsCount = GetContactListCount($proposal->rel_id);
                                if($ProAcceptCount >= $ContactsCount){
                    ...
                    if ($success) {
                        process_digital_signature_image($this->input->post('signature', false), PROPOSAL_ATTACHMENTS_FOLDER . $id);
                        $data = get_acceptance_info_array();
                        $this->db->update(db_prefix().'signiture_details', $data);
                        $this->db->where('id', $id);
        ...
        $data['proposalAcceptList']  = $proposalAcceptList;
