...
class Payments_model extends App_Model
{
    ...
    public function add($data, $subscription = false)
    {
        ...
        if (isset($data['TdsApplicable'])) {
            $TdsApplicable = $data['TdsApplicable'];
            unset($data['TdsApplicable']);
        }
        if (isset($data['tds_amount'])) {
            $tds_amount = $data['tds_amount'];
            unset($data['tds_amount']);
        }
        ...
        if ($insert_id) {
            // Tds Payment record add
            if($TdsApplicable == "Y"){
                $data["amount"] = $tds_amount;
                $data["paymentmode"] = "TDSR";// TDS Receivable
                $this->db->insert(db_prefix() . 'invoicepaymentrecords', $data);
            }
