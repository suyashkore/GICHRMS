    ...
    public function get($id = '', $where = [], $for_editor = false)
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($contract) {
                ...
                $OtherDetails = $this->GetOtherDetails($id);
                $MilestoneDetails = $this->GetMilestoneDetails($id);
                $contract->OtherDetails = $OtherDetails;
                $contract->MilestoneDetails = $MilestoneDetails;
...
//======================== Get Contract Other Details ==========================
    public function GetOtherDetails($id)
    {
        $this->db->select('tblcontractsDetails.*');
        $this->db->where(db_prefix() . 'contractsDetails.ContractID', $id);
        $contractDetails = $this->db->get(db_prefix() . 'contractsDetails')->row();
        return $contractDetails;
    }
    
//======================== Get Contract Milstone Details =======================
    public function GetMilestoneDetails($id)
    {
        $this->db->select('tblcontractsMilestoneList.*');
        $this->db->where(db_prefix() . 'contractsMilestoneList.ContractID', $id);
        $contractmilestone = $this->db->get(db_prefix() . 'contractsMilestoneList')->result_array();
    ...
    public function add($data)
    {
        ...
        $VideoCreation = $data["VideoCreation"];
        $Keywords = $data["Keywords"];
        $Blogs = $data["Blogs"];
        $Service = $data["Service"];
        $Min_Working_Day = $data["Min_Working_Day"];
        $ServiceDescription = $data["ServiceDescription"];
        $remark_a_smo = $data["remark_a_smo"];
        $remark_b_smo = $data["remark_b_smo"];
        unset($data['DescriptionList'],$data['MilestoneAmt'], $data['MonthlyPayment'], $data['MonthlyDueDate'],$data['AMC_Charges']);
        unset($data['ProjectDuration'],$data['Facebook'], $data['LinkedIn'], $data['Instagram'],$data['GMB']);
        unset($data['YouTube'],$data['VideoCreation'], $data['Keywords'], $data['Blogs'],$data['Service']);
        unset($data['Min_Working_Day'],$data['ServiceDescription'], $data['remark_a_smo'], $data['remark_b_smo']);
        $proposal_details = array(
            "MonthlyPayment"=>$MonthlyPayment,
            "MonthlyDueDate"=>to_sql_date($MonthlyDueDate),
            ...
            "Blogs"=>$Blogs,
            "Service"=>$Service,
            "Min_Working_Day"=>$Min_Working_Day,
            "ServiceDescription"=>$ServiceDescription,
            "remark_a_smo"=>$remark_a_smo,
            "remark_b_smo"=>$remark_b_smo,
        );
        /*echo "<pre>";
        print_r($proposal_details);
        print_r($data);
        die;*/
        if (isset($data['ContractDate'])) {
            $data['ContractDate'] = to_sql_date($data['ContractDate']);
        }
        ...
        if ($insert_id) {
            // Porposal other details
            $proposal_details["ContractID"] = $insert_id;
            $this->db->insert(db_prefix() . 'contractsDetails', $proposal_details);
            // Milestone list
            $i = 0;
            foreach($DescriptionList as $val){
                $insert = array(
                    "ContractID"=>$id,
                    "description"=>$val,
                    "amount"=>$MilestoneAmt[$i]
                );
                $i++;
                $this->db->insert(db_prefix() . 'contractsMilestoneList', $insert);
            }
    ...
    public function update($data, $id)
    {
        ...
        if($data["MonthlyDueDate"]){
            $OtherDetails['MonthlyDueDate'] = to_sql_date($data['MonthlyDueDate']);
        }
        unset($data['DescriptionList'],$data['MilestoneAmt'], $data['MonthlyPayment'], $data['MonthlyDueDate'],$data['AMC_Charges'],
        $data['ProjectDuration'],$data['Facebook'],$data['LinkedIn'],$data['Instagram'],$data['GMB'],
        $data['YouTube'],$data['VideoCreation'],$data['Keywords'],$data['Blogs'],$data['Service'],
        $data['Min_Working_Day'],$data['ServiceDescription'],$data['remark_a_smo'],$data['remark_b_smo']);
        ...
        //if ($this->db->affected_rows() > 0) {
            ...
            $this->db->delete(db_prefix() . 'contractsDetails');
            $this->db->insert(db_prefix() . 'contractsDetails', $OtherDetails);
            // update milestone details
            $this->db->where('ContractID', $id);
            $this->db->delete(db_prefix() . 'contractsMilestoneList');
            $i = 0;
            foreach($DescriptionList as $val){
                $insert = array(
                    "ContractID"=>$id,
    ...
    {
        $contract = $this->get($id, [], true);
        $data = get_acceptance_info_array();
        $data["rel_id"] = $id;
        $data["rel_type"] = "contract";
        if($this->db->insert(db_prefix().'signiture_details', $data)){
            $ConAcceptCount = GetContractAccenptCount($id);
            $ContactsCount = GetContactListCount($contract->client);
            $content = override_merge_fields($this->get_merge_fields($contract), $contract->content);
            $contract_array = array(
                "content"   =>$content
            );
            if($ConAcceptCount >= $ContactsCount){
                $contract_array["signed"] = 1;
            }
            $this->db->where('id', $id);
            $this->db->update(db_prefix().'contracts', $contract_array);
            // Notify contract creator that customer signed the contract
            send_contract_signed_notification_to_staff($id);
            return true;
        }
    ...
    public function GetAcceptList($id)
    {   
        $this->db->where('rel_id', $id);
        $this->db->where('rel_type', 'contract');
        $result = $this->db->get(db_prefix() . 'signiture_details');
        return $result->result_array();
    }
    public function clear_signature_all($id)
    {
        $this->db->select('*');
        $this->db->where('rel_id', $id);
        $this->db->where('rel_type', "contract");
        $proposalsign_list = $this->db->get(db_prefix() . 'signiture_details')->result_array();
        $contractData = $this->get($id, [], true);
        ...
        if($proposalsign_list){
            foreach($proposalsign_list as $key=>$val) {
                $this->db->where('id', $val['id']);
                $this->db->update(db_prefix() . 'signiture_details', ['signature' => null]);
    
                if (!empty($proposal->signature)) {
                    unlink(get_upload_path_by_type('contract') . $id . '/' . $val["signature"]);
                }
            }
            return true;
        }
