...
// Signed List
// Contract Acept List
$AcceptContractList = GetContractAcceptList($proposal->id);
$AcceptHtml = "";
    //$items_html .= '<table cellpadding="6" style="font-size:' . ($font_size + 4) . 'px">';
    foreach($AcceptContractList as $key=>$val){
        $url = site_url('download/preview_image?path=' . protected_file_url_by_path(get_upload_path_by_type('proposal') . $proposal->id . '/' . $val["signature"]));
        
        $AcceptHtml .= '<p class="text-muted proposal-signed-by"><b>'._l('proposal_signed_by').' : </b>'.e("{$val['acceptance_firstname']} {$val['acceptance_lastname']}").'</p>';
        
        $AcceptHtml .= '<p class="text-muted proposal-signed-by"><b>'._l('proposal_signed_date').' : </b>'.e(_d(explode(' ', $val["acceptance_date"])[0])).'</p>';
        
        $AcceptHtml .= '<p class="text-muted proposal-signed-by"><b>'._l('proposal_signed_ip').' : </b>'.e($val['acceptance_ip']).'</p>';
       
        $AcceptHtml .= '<p class="text-muted proposal-signed-by"><img src="'.$url.'" class="img-responsive" alt=""></p>';
        $AcceptHtml .= '<hr>';   
    }
    //$items_html .= '</table>';
    $proposal->content = $proposal->content." ".$AcceptHtml;
