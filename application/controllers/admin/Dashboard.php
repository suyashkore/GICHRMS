...
class Dashboard extends AdminController
{
    ...
    {
        error_log("Dashboard index method started");
        close_setup_menu();
        ...
        $data['calendar_view'] = $this->dashboard_model->get_calendar_view(get_staff_user_id(), date('Y-m'));
        $data['today_attendance'] = $this->dashboard_model->get_day_status(get_staff_user_id(), date('Y-m-d'))['data'];

        $data = hooks()->apply_filters('before_dashboard_render', $data);
        error_log("About to load dashboard view");
    ...
    public function get_day_status($date)
    {
        if ($this->input->is_ajax_request()) {
            echo json_encode($this->dashboard_model->get_day_status(get_staff_user_id(), $date));
